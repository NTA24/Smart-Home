...
import { StatCard, LineChart, BarChart, GaugeChart, PageContainer, PageHeader, ContentCard, DataTable } from '@/components'
...
export default function EnergyMonitor() {
  ...
  const columns = [
    ...
    { title: t('energy.vsPrevious'), dataIndex: 'change', key: 'change', render: (val: string) => <span className={val.startsWith('+') ? 'other_change-positive' : val.startsWith('-') ? 'other_change-negative' : 'other_change-neutral'}>{val}</span> },
  ]
  ...
  return (
    <PageContainer>
      ...
      <ContentCard title={t('energy.areaDetails')}>
        <DataTable columns={columns} dataSource={consumptionData} pagination={false} />
      </ContentCard>
    </PageContainer>
  )
}
