...
export default function ElevatorAccessControl() {
    ...
    },
    {
      ...
      render: (exceptions: string[]) =>
        exceptions.length > 0 ? (
          ...
          <Text type="secondary" className="text-11">—</Text>
        ),
    },
    {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={t('elevatorAccess.title')}
        icon={<SafetyCertificateOutlined />}
        subtitle={selectedBuilding?.name || t('elevatorAccess.allSites')}
        actions={
          <>
      ...
      <Row gutter={16}>
        ...
        <Col xs={24} md={6}>
          <ContentCard
            title={t('elevatorAccess.zonesFloors')}
          ...
          >
            ...
            />
            {selectedZoneFloors.length > 0 && (
              <div className="elevator_filter-active-box">
                <Text className="text-11 text-primary">
          ...
          <ContentCard
            title={
              <>
                ...
                <Tag color="blue" className="rounded">{filteredRules.length}</Tag>
              </>
            }
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} layout="vertical" className="mt-16" initialValues={{ subjectType: 'group', method: t('elevatorAccess.methodCard'), isTemporary: false, enabled: true }}>
          {/* Subject */}
          <Row gutter={12}>
            <Col span={8}>
              <Form.Item name="subjectType" label={t('elevatorAccess.subjectType')} rules={[{ required: true }]}>
                <Select>
                  <Select.Option value="group"><TeamOutlined className="mr-4" />{t('elevatorAccess.group')}</Select.Option>
                  <Select.Option value="user"><UserOutlined className="mr-4" />{t('elevatorAccess.user')}</Select.Option>
                </Select>
              </Form.Item>
            </Col>
            <Col span={16}>
