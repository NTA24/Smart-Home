...
import { PageContainer, PageHeader, ContentCard, DataTable, DetailDrawer, StatusTag } from '@/components'
...
export default function ElevatorAlarms() {
  ...
  const columns = [
    {
      ...
      key: 'time',
      width: 80,
      render: (time: string) => <Text className="text-base font-mono">{time}</Text>,
    },
    {
      ...
      key: 'elevator',
      width: 70,
      render: (e: string) => <Text strong className="font-mono">{e}</Text>,
    },
    {
    ...
    },
    {
      ...
      key: 'location',
      width: 120,
      render: (loc: string) => <Text className="text-sm">{loc}</Text>,
    },
    {
      ...
      render: (status: AlarmStatus) => (
        <StatusTag status={status} config={STATUS_CONFIG} />
      ),
    },
    {
      title: 'SLA',
      ...
      render: (sla: string, record: ElevatorAlarm) => (
        <Text className="font-mono text-base font-semibold" style={{ color: record.slaExpired ? '#f5222d' : record.status === 'resolved' ? '#52c41a' : '#1a1a1a' }}>
          <ClockCircleOutlined className="mr-4 text-11" />
          {sla}
          {record.slaExpired && <WarningOutlined className="ml-4 text-xs text-danger" />}
        </Text>
      ),
    },
    {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={t('elevatorAlarms.title')}
        icon={<AlertOutlined />}
        subtitle={selectedBuilding?.name || t('elevatorAlarms.allSites')}
      ...
      >
        {selectedAlarm && (
          <>
            {/* Description */}
            <Card size="small" className="rounded-md mb-16" style={{ background: SEVERITY_CONFIG[selectedAlarm.severity].bg, border: `1px solid ${SEVERITY_CONFIG[selectedAlarm.severity].color}30` }}>
              <Text className="text-base">{selectedAlarm.description}</Text>
            </Card>
            ...
            <Steps
              ...
              items={selectedAlarm.checklist.map(step => ({
                title: <Text className="text-base">{step.step}</Text>,
                description: <Tag color={step.role === 'operator' ? 'blue' : 'purple'} className="rounded-sm text-xs">{step.role === 'operator' ? t('elevatorAlarms.roleOperator') : t('elevatorAlarms.roleSecurity')}</Tag>,
            ...
            <Row gutter={[10, 10]} className="mb-16">
              <Col span={12}>
                <Button icon={<PhoneOutlined />} block className="rounded h-10" style={{ borderColor: '#1890ff', color: '#1890ff' }}>
                  {t('elevatorAlarms.callIntercom')}
                </Button>
              </Col>
              <Col span={12}>
                <Button icon={<BellOutlined />} block className="rounded h-10" style={{ borderColor: '#fa8c16', color: '#fa8c16' }}>
            ...
            <div className="overflow-y-auto mb-12 max-h-180">
              {selectedAlarm.notes.length > 0 ? selectedAlarm.notes.map((note, i) => (
                <div key={i} className="elevator_note-item">
                  <div className="flex-between mb-2">
                    <Text strong className="text-sm">{note.user}</Text>
                    <Text className="text-11 text-muted font-mono">{note.time}</Text>
                  </div>
                  <Text className="text-base">{note.text}</Text>
                </div>
              )) : (
                <Text type="secondary" className="text-sm">{t('elevatorAlarms.noNotes')}</Text>
              )}
            </div>
            <Space.Compact className="w-full mb-8">
              <Input
            ...
            <Upload maxCount={3} beforeUpload={() => false}>
              <Button icon={<UploadOutlined />} size="small" className="rounded-sm">
              ...
              <Col span={12}>
                <Popconfirm title={t('elevatorAlarms.confirmResolve')} onConfirm={handleResolve} okText={t('apiTest.yes')} cancelText={t('apiTest.no')}>
                  <Button
                    ...
                    block
                    size="large"
                    className="elevator_resolve-btn"
                  >
              ...
              <Col span={12}>
                <Popconfirm title={t('elevatorAlarms.confirmEscalate')} onConfirm={handleEscalate} okText={t('apiTest.yes')} cancelText={t('apiTest.no')}>
                  <Button
                    ...
                    block
                    size="large"
                    className="elevator_escalate-btn"
                  >
