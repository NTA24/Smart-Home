...
const DIRECTION_ICON: Record<Direction, React.ReactNode> = {
  up: <ArrowUpOutlined className="text-success" />,
  down: <ArrowDownOutlined className="text-primary" />,
  idle: <MinusOutlined className="text-muted" />,
}
...
export default function ElevatorLive() {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={t('elevatorLive.title')}
        icon={<ArrowUpOutlined />}
        subtitle={selectedBuilding?.name || t('elevatorLive.allSites')}
        actions={<Button icon={<ReloadOutlined />}>{t('parkingMap.refresh')}</Button>}
      />

      {/* Filters */}
      <ContentCard style={{ marginBottom: 16 }} bodyStyle={{ padding: '12px 20px' }}>
        ...
        {filtered.map(elevator => (
          <Col key={elevator.id} xs={24} sm={12} lg={8} xl={6}>
            ...
            >
              ...
              {/* Floor + Direction */}
              <div className="elevator_card-floor-box">
                <div className="elevator_card-floor-value">
                  {elevator.floorLabel}
                </div>
        ...
        title={
          selectedElevator ? (
            <div className="flex items-center gap-12">
              <Text strong className="text-2xl font-mono">{selectedElevator.name}</Text>
              <Tag color={STATUS_COLOR[selectedElevator.status]} className="rounded">
                {selectedElevator.status}
              </Tag>
              <Tag color={MODE_CONFIG[selectedElevator.mode].color} className="rounded">
      ...
      >
        {selectedElevator && (
          <>
            {/* Live status */}
            <Card size="small" className="elevator_drawer-status-card">
              <Row gutter={[16, 8]}>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.currentFloor')}</Text>
                  <div className="text-4xl font-bold font-mono">
                    {selectedElevator.floorLabel} {DIRECTION_ICON[selectedElevator.direction]}
                  </div>
                </Col>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.load')}</Text>
                  <div>
                    <Text strong className="text-2xl" style={{ color: LOAD_COLOR(selectedElevator.load) }}>{selectedElevator.load}%</Text>
                  </div>
                  <Progress percent={selectedElevator.load} showInfo={false} size="small" strokeColor={LOAD_COLOR(selectedElevator.load)} />
                </Col>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.door')}</Text>
                  <div>
                    <Tag color={selectedElevator.door === 'open' ? 'green' : 'default'} className="rounded-sm">
                      {selectedElevator.door === 'open' ? t('elevatorLive.doorOpen') : t('elevatorLive.doorClosed')}
                    </Tag>
                  </div>
                </Col>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.queue')}</Text>
                  <div><Text strong className="text-lg">{selectedElevator.queue}</Text></div>
                </Col>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.floorRange')}</Text>
                  <div><Text className="text-base font-mono">{selectedElevator.floorRange[0]} → {selectedElevator.floorRange[1]}</Text></div>
                </Col>
                <Col span={12}>
                  <Text type="secondary" className="text-11">{t('elevatorLive.bank')}</Text>
                  <div><Tag color="blue" className="rounded-sm">{t('elevatorLive.bank')} {selectedElevator.bank}</Tag></div>
                </Col>
              </Row>
            </Card>
            ...
            <Divider className="mt-12 mb-12">{t('elevatorLive.quickActions')}</Divider>

            <Space direction="vertical" className="w-full" size={10}>
              <Button icon={<EyeOutlined />} block className="elevator_action-btn">
                {t('elevatorLive.viewDetail')}
              </Button>
              <Button icon={<StopOutlined />} block danger className="elevator_action-btn">
                {t('elevatorLive.setOutOfService')}
              </Button>
              <Button icon={<SoundOutlined />} block className="elevator_action-btn" style={{ borderColor: '#fa8c16', color: '#fa8c16' }}>
                {t('elevatorLive.announceMessage')}
              </Button>
              <Button icon={<ToolOutlined />} block className="elevator_action-btn" style={{ borderColor: '#722ed1', color: '#722ed1' }}>
