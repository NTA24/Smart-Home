...
const RobotMaintenance = () => {
  ...
  const healthIcon: Record<string, React.ReactNode> = {
    ...
    critical: <ExclamationCircleOutlined className="text-danger" />,
  }

  /* ── Work order columns ── */
  const woCols: ColumnsType<WorkOrder> = [
    { title: 'ID', dataIndex: 'id', width: 90, render: (v: string) => <Text strong className="text-sm">{v}</Text> },
    ...
    { title: t('robotMaint.woType'), dataIndex: 'type', width: 160 },
    {
      ...
      render: (v: string) => { const c = prioConfig[v]; return <Tag color={c.color} className="rounded-sm m-0">{c.label}</Tag> },
    },
    {
      title: t('robotAlerts.colStatus'), dataIndex: 'status', width: 120,
      render: (v: string) => { const c = statusConfig[v]; return <Tag color={c.color} className="rounded-sm m-0">{c.label}</Tag> },
    },
  ...
  return (
    <PageContainer>
      <PageHeader title={t('robotMaint.title')} />

      <Row gutter={[12, 12]} className="mb-16 flex">
        {/* Health */}
        <Col xs={24} sm={8} className="flex">
          <ContentCard className="robot_health-card" bodyStyle={{ padding: 16 }}>
            <div className="robot_section-title">
              <Text strong className="text-md"><RobotOutlined style={{ marginRight: 6 }} />{t('robotMaint.health')}</Text>
            </div>
            <Space size={12} wrap>
              <Tooltip title="OK">
        ...
        <Col xs={24} sm={8} style={{ display: 'flex' }}>
          <ContentCard style={{ borderRadius: 10, width: '100%' }} bodyStyle={{ padding: 16 }}>
      ...
      <ContentCard
        ...
        bodyStyle={{ padding: 0 }}
        title={t('robotMaint.robotHealth')}
        titleIcon={<RobotOutlined />}
      >
        <DataTable<RobotHealth>
