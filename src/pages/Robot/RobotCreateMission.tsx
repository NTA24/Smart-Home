...
const RobotCreateMission = () => {
  ...
  return (
    <PageContainer>
      <PageHeader title={t('robotMission.title')} />

      <Form form={form} layout="vertical" initialValues={{ schedule: 'now', priority: 'high', sla: 10 }}>
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={16}>
            <ContentCard className="rounded-md" bodyStyle={{ padding: 20 }}>
              {/* Type, Priority, Schedule */}
              <Row gutter={16}>
                <Col xs={24} sm={8}>
                  <Form.Item
                    name="type"
                    label={<span><FlagOutlined className="mr-4" />{t('robotMission.type')}</span>}
                ...
                <Col xs={24} sm={8}>
                  <Form.Item
                    name="priority"
                    label={<span><ThunderboltOutlined className="mr-4" />{t('robotMission.priority')}</span>}
                    rules={[{ required: true }]}
                  >
                    <Select
                      options={priorities.map(p => ({
                        value: p.value,
                        label: <Tag color={p.color} className="m-0 rounded-sm">{p.label}</Tag>,
                      }))}
                    />
                  </Form.Item>
                </Col>
                <Col xs={24} sm={8}>
                  <Form.Item
                    name="schedule"
                    label={<span><ClockCircleOutlined className="mr-4" />{t('robotMission.schedule')}</span>}
                  >
          ...
          <Col xs={24} lg={8}>
            <ContentCard style={{ borderRadius: 10, position: 'sticky', top: 16 }} bodyStyle={{ padding: 20 }}>
              <Title level={5} style={{ marginTop: 0 }}>{t('robotMission.summary')}</Title>
              <div className="robot_summary-box">
                <Form.Item noStyle shouldUpdate>
                  {() => {
                    ...
                    return (
                      <div className="robot_summary-row">
