...
import { PageContainer, PageHeader, ContentCard, DataTable, DetailDrawer, SearchInput } from '@/components'
import { getRobotAlertsFilters, saveRobotAlertsFilters } from '@/services/mockPersistence'
...
const RobotAlerts = () => {
  ...
  const persistedFilters = getRobotAlertsFilters<{ selectedTab: string; keyword: string; status: string }>({
    selectedTab: 'all',
    keyword: '',
    status: 'all',
  })
  const [selectedTab, setSelectedTab] = useState<string>(persistedFilters.selectedTab)
  const [keyword, setKeyword] = useState<string>(persistedFilters.keyword)
  const [statusFilter, setStatusFilter] = useState<string>(persistedFilters.status)
  ...
  useEffect(() => {
    saveRobotAlertsFilters({ selectedTab, keyword, status: statusFilter })
  }, [selectedTab, keyword, statusFilter])

  const filteredAlerts = useMemo(() => {
    const normalized = keyword.trim().toLowerCase()
    return alerts.filter((alert) => {
      const matchTab = selectedTab === 'all' || alert.severity === selectedTab
      ...
      const matchKeyword = !normalized
        ...
        || alert.robot.toLowerCase().includes(normalized)
        || alert.type.toLowerCase().includes(normalized)
        || alert.location.toLowerCase().includes(normalized)
      return matchTab && matchStatus && matchKeyword
    })
  }, [alerts, selectedTab, statusFilter, keyword])
    ...
    },
    {
      ...
      render: (v: string, rec: RobotAlert) => (
        <Text style={{ color: rec.severity === 'critical' ? '#ff4d4f' : '#595959', fontWeight: rec.severity === 'critical' ? 600 : 400 }}>
          <ClockCircleOutlined className="mr-4" />{v}
        </Text>
      ),
    },
    {
      ...
      render: (v: string) => {
        ...
        return <Tag color={c.color} className="rounded-sm">{c.label}</Tag>
      },
    },
    {
  ...
  return (
    <PageContainer>
      <PageHeader title={t('robotAlerts.title')} />
      ...
      <Space size={8} style={{ marginBottom: 12 }}>
        ...
        <Select
          ...
          options={[
            ...
            { value: 'active', label: t('robotAlerts.statusActive') },
            { value: 'acknowledged', label: t('robotAlerts.statusAck') },
            { value: 'resolved', label: t('robotAlerts.statusResolved') },
          ]}
        />
      </Space>

      <ContentCard style={{ borderRadius: 10 }} bodyStyle={{ padding: 0 }}>
        <DataTable<RobotAlert>
        ...
        {selectedAlert && (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div className="robot_checklist-box">
